...
- ACCESS TOKENS are an object that describes and identifies the security context of a process or thread on a system.
- The access token is generated by the WINLOGON PROCESS everytime a user auths successfully, and includes the identity and privileges of the user account associated with the thread or process.
- The token is attached to the initial process (typically userinit.exe) after which all child processes will inherit a copy of the access token from their creator and will run under the same access token.

On Windows, an access token will compromise the following:
- User SID
- Group SID
- Logon SID
- Privs assigned to the user or user group
- DISCRETIONARY ACCESS CONTROL LIST (DACL)
- Source of the access token

We can list out the access token of a user by running:
```
whoami \priv
```
